...
每个真实验证 workflow 应提供 `appId` 和 `goal`，便于知识库预检、report 审计和执行后回写。`learn` 可以是 `true` 或对象，例如：
...
每轮验证后的基础知识回写应启用 `learn`。命令行 `ev_workflow.py --learn` 也可以启用基础学习；如需 action/workflow 资产，命令行显式加 `--learn-assets`，或在 workflow 的 `learn` 对象中设置 `includeAssets: true`。命令行传入的 learn 选项优先生效。
...
当命令行或 workflow 提供 `appId` / `goal` 时，脚本会默认启用基础知识回写；如果确实要跳过，必须显式传 `--no-learn`，并在最终回复中说明原因。`--no-learn` 不能和 `--learn`、`--learn-assets` 同时使用。
