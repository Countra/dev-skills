...
先运行 `ev_doctor.py`。如果 server 离线，按 `references/server.md` 的统一流程使用 process-manager status/start/ready，不要手写后台命令或判断 OS/backend。只有 process-manager 自身失败且原因不清楚时才运行 `pm_doctor.py`。
