...
8. 知识库预检后必须执行 `Reuse Gate`：如果命中可执行 workflow asset、action asset 或已批准 workflow 文件，优先用 `ev_workflow.py --workflow-id`、`ev_action.py --action-id` 或原 workflow 文件现场复验；不得导出、复制或手写等价 JSON。
9. 只有命中为空、命中不可执行、风险过高、目标不匹配或现场复验失败时，才允许创建新的 action/workflow；最终回复必须说明没有复用的原因。
10. 现场验证必须复用稳定 session，通过 `ev_action.py` 或 `ev_workflow.py` 执行本轮实际操作；默认只生成 report、artifact 和 pending 审核包，不自动固化正式 workflow，也不自动写入知识库。
11. 自动化过程中如果进入错误页面、点击无关功能、找不到目标入口或流程与用户目标不符，必须自我纠偏并把弯路标记为 detour；最终待确认和可持久化的 workflow 只能包含清洗后的正确路径。
12. 现场验证后必须把正确路径、已排除的弯路、证据路径和 pending 审核包路径在会话中说明清楚，并询问用户是否允许持久化；用户确认前不得保存正式 workflow，不得写入知识库。
13. 用户明确批准后，才允许通过 `ev_persist.py approve` 将 pending workflow 晋级到 `.harness/electron-ui-verifier/workflows/`，并按批准范围写入基础知识或 action/workflow 资产。
14. 本 skill 自己生成和托管的内部运行文件必须位于 `.harness/electron-ui-verifier/` 下，包括 environment、config、token、server、sessions、reports、pending、workflows、artifacts、logs、tmp 和 knowledge；不要自行写到其它 `.harness` 子目录、项目根目录、`skills/` 目录或临时目录。
15. 最终回复必须汇报知识库预检命中情况、复用的 workflow/action asset ID 或已批准 workflow 文件、未复用候选及原因、新生成文件的原因、session、target、pending 审核包或已批准 workflow 路径、是否已写入知识库、已排除的弯路、截图、抽取内容、report 路径、artifact 路径和未覆盖范围。没有证据产物时，不得声称真实 UI 验证通过。
...
- 最终回复必须包含知识库预检、资产复用和持久化摘要；用户未确认前必须明确说明“未写入知识库、未保存为长期 workflow 资产”。
