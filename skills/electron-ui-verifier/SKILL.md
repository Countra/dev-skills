...
1. 启动、连接或验证 Electron 应用前，先读取 `references/server.md` 和 `references/workflow.md`。
...
7. 汇报实际使用的 session、target、截图、抽取内容、report 路径、artifact 路径和未覆盖范围。没有证据产物时，不得声称真实 UI 验证通过。
