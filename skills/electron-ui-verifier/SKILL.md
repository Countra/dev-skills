...
7. 知识库默认不自动写入；只有显式使用 `ev_learn.py` 或 `--learn` 时才沉淀候选知识。
8. 汇报实际使用的 session、target、截图、抽取内容、report 路径、artifact 路径和未覆盖范围。没有证据产物时，不得声称真实 UI 验证通过。
...
- `scripts/ev_learn.py`: 从 report 显式学习候选知识。
