...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        options = {"maxEvents": args.max_events, "includeFailedOnly": args.failed_only}
        if args.url_contains:
            options["urlContains"] = args.url_contains
        action = {"id": args.id, "type": "collectNetwork", "options": options, "continueOnFailure": True}
