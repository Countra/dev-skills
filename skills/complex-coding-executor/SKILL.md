...
- contract 的 dependency mode 非 `none` 时读取 `references/dependency-execution.md`：preflight 校验批准证据新鲜度和 stage 映射，涉及 manifest/lock 的阶段用生态原生命令生成 task-local runtime receipt；身份、版本策略、路径、hard gate 或 advisory 漂移不得静默放行。
