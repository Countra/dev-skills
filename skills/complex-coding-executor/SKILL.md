...
4. required validation 从 contract 读取 `command` 与 `timeout_seconds`，按无 shell argv 规则经 `harness_bounded_command.py` 执行，再用 `validate` 记录最近结果和简短摘要，不保存完整日志。
...
- 快速、确定范围的本地读取可以使用宿主短 deadline。系统 provider、进程枚举、包管理器、测试、构建、已有卡死历史或可能长时间静默的有限命令，若宿主不能证明会按时回收其进程边界，必须实际以 `harness_bounded_command.py` 作为最外层 program；不要只在答复中建议。
- contract validation 的 `timeout_seconds` 必须传给 helper，默认 grace 为 5 秒；`124/125/126/130` 都记录为 failed。命令只按单个 ASCII 空格拆成无 shell argv，复杂逻辑先放入受版本控制的脚本。
- 宿主使用不超过 10 秒的初始观察窗口和固定绝对 deadline，轮询不能重置预算。PowerShell 自动化使用 `-NoProfile -NonInteractive`；不要用 `Tee-Object` 保存测试日志，也不要用无界全系统进程扫描判断状态。
