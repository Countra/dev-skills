...
6. 按 standards index 和 Development Quality Gate 实施静态检查、架构边界复核及 required VAL，写带结果摘要的 `validation_recorded`；大证据使用 task-dir 相对 evidence refs。
7. 用 `complex-coding-reviewer` 的 `code-review/stage-delta` 审查当前 stage target，把 canonical JSON receipt 写入 `artifacts/reviews/**`，再记录由公共 validator 精确派生的 compact `review_recorded`。
...
3. 使用 Reviewer 公共 `review_validate.py` 校验 profile=`code-review`、scope=`stage-delta`、stage/attempt、supersedes 和 freshness。
4. `review_recorded` 的 `attempt` 必须与当前 attempt 一致；`evidence_refs` 必须包含 receipt 的 task-relative `report_ref`。
5. event writer 会再次调用 Reviewer 公共 CLI，并要求 payload 与校验结果精确一致；它还会核对 canonical paths、target mode 与当前 `HEAD`，`stage_completed` 前重建 target，任一目标变化即拒绝回执。
...
1. 确认每个非 carried stage 都有 required VAL、当前 attempt 的 `stage-delta` receipt 和 `stage_completed` evidence；carried stage 以已校验 amendment archive 为证据。
...
4. 对 execution baseline 到当前整体目标运行 `code-review/final-integration`；修复 blocking/major 后重跑受影响验证并生成新 receipt。
