...
- 成功提交后写 `commit_recorded`，payload 必须包含 `commit` hash 和 `repository`，可按需补充 message/scope；不要伪造未发生的提交。新 `review_recorded` 会自动绑定可选 `report_digest`，历史事件保持原样。
- final commit 后可先运行 `harness_commit_equivalence.py --workspace <workspace> --task-dir <task-dir> --format json create`。成功结果给出完整 `commit_payload` 与 `evidence_refs`，只能原样用于随后一次 final `commit_recorded`；helper 不提交代码、不写 ledger，也不生成语义 verdict。
- equivalence 仅接受提交前 strict 独立 final receipt，并重算单提交父关系、baseline、allowed paths、A/M/D、删除项、被审工作树字节、Git clean-filter 后 blob、canonical commit target 和 source 工作树清洁度。失败时不要修改 proof 或强行复用 receipt，改写普通 final `commit_recorded` 并执行既有 post-commit strict review。
...
5. 未授权时不得提交。已授权且 contract 预期 final commit 时，先用当前 strict final receipt 决定是否可提交；实际提交后优先尝试一次确定性 commit-equivalence。proof 成功时把其 ref/digest 和 supporting target 一并写入无 `stage_id` 的 `commit_recorded`，状态机保留被精确绑定的 pre-commit receipt。
6. equivalence 因旧 receipt、hook/部分提交、额外未跟踪文件、内容/状态/范围漂移或任何校验失败而不可用时，按旧路径写入不含 proof 的 `commit_recorded`；该事件撤销 pre-commit receipt。随后以 execution baseline、全部 canonical allowed paths 和当前 `HEAD` 生成真实 `commit-range`，重新记录 strict `final-integration` receipt。不得手工补 proof，也不得复用旧 HEAD 或 stage receipt。
