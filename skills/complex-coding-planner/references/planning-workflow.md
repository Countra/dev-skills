...
| plan-review receipt | 必需，focused lenses | 必需 | 必需，优先更强 provenance |
...
10. 完成 Plan Quality Gate 与 Producer Readiness Gate，写入 approval-included review brief，冻结 plan target/context，并在 contract 中索引当前 brief 与递增的 receipt attempt 路径。
11. 调用 `complex-coding-reviewer` 的 `plan-review`；Reviewer 只写 canonical receipt，Planner 根据 finding 修复目标并在需要时重建双 target、进入下一 attempt。
12. 运行 planner checker `approval`，由其调用 Reviewer 公共 validator 校验 profile、scope、coverage/gap/lineage、双 target freshness 和 supersedes，并拒绝 context 中未进入批准集合的文件。
...
- Reviewer 对当前 plan-bundle 与 context 生成 coverage、strengths、findings、verification gaps、lineage 和 verdict。same-context、fresh-context、external-agent 或 human 只按真实 provenance 记录，skill 名称本身不构成独立性。
...
- planner `approval` checker 只接受 Reviewer CLI 验证后的 `passed` receipt；missing、wrong-profile、wrong-scope、coverage/gap/lineage 不完整、open major、blocked lens、断裂 supersedes、stale target/context 或 unapproved context path 均 fail closed。
