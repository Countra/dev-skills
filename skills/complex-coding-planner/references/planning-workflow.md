...
10. 完成 Plan Quality Gate、Plan Self-Review，必要时做独立 critique。
11. 运行 planner checker `approval` 并完成 Readiness Gate。
12. 通过 active-task helper 安全写入 pointer，请求用户批准后停止。
...
2. 运行 `harness_active_task.py --workspace <workspace> --task-dir <task-dir> --mode activate`，由 helper 原子写 pointer-only `.harness/active-task.json`。
