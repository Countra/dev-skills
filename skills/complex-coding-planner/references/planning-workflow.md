...
10. 完成 Plan Quality Gate 与 Producer Readiness Gate，冻结当前 review target，并在 contract 中索引递增的 plan-review attempt 路径。
11. 调用 `complex-coding-reviewer` 的 `plan-review`；Reviewer 只写 canonical receipt，Planner 根据 finding 修复目标并在需要时进入下一 attempt。
12. 运行 planner checker `approval`，由其调用 Reviewer 公共 validator 校验 profile、scope、verdict、target freshness 和 supersedes。
...
- 所有 managed plan 必须只索引当前 `artifacts/reviews/plan-review-attempt-N.json`；历史 attempts 留在目录中但不进入当前 artifact index。
...
- Reviewer 对当前 plan-bundle 生成 verdict 和 findings。same-context、fresh-context、external-agent 或 human 只按真实 provenance 记录，skill 名称本身不构成独立性。
- Planner 只消费 finding 并修复 plan/contract/artifacts。任何目标变化都会使旧 digest stale；先把 contract 指向下一 attempt，再重跑 draft checker 和 review。
- planner `approval` checker 只接受 Reviewer CLI 验证后的 `passed` receipt；missing、wrong-profile、wrong-scope、open major、blocked lens、断裂 supersedes 或 stale target 均 fail closed。
