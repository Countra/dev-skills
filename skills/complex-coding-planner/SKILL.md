...
规划调研命令也必须有停止边界。快速本地读取可使用宿主短 deadline；系统 provider、进程枚举、包管理器、测试、构建、已有卡死历史或可能长时间静默的有限命令，必须遵循 Executor 的 bounded-command 规则。不得用无界诊断来制定“稳定性”方案。
...
- required validation 的 `timeout_seconds` 必须成为实际执行预算。命令使用无 shell、单 ASCII 空格分隔的 program/args；管道、重定向、变量展开或复杂 PowerShell/sh 逻辑先写入受版本控制的脚本。
