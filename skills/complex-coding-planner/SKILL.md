...
- 所有 managed profile 都必须索引一个 required、approval-included 的 `artifacts/reviews/plan-review-attempt-N.json`。approval checker 通过 Reviewer 公共 CLI 验证 `managed-plan` scope、passed verdict、当前 target digest 和连续 supersedes；不读取 Markdown 关键词。
