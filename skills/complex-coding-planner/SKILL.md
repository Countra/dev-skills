...
- 提交审批前完成 Plan Quality Gate 与 Producer Readiness Gate，再调用 `complex-coding-reviewer` 的 `plan-review`；Planner 根据 finding 修复目标，但不得自行生成正式 verdict。
- 所有 managed profile 都必须索引 approval-included 的 review brief 和 required plan-review receipt。approval checker 通过 Reviewer 公共 CLI 验证 `managed-plan` scope、target/context 双 digest、coverage、gap、lineage、passed verdict 和连续 supersedes；context 只能引用 plan、contract 与 approval-included planning artifacts，不读取 Markdown 关键词。
