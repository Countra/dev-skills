...
所有落盘写文件动作都适用该策略，包括代码、Markdown 文档、规划文档、模板、JSON、JSONL、YAML、changelog、eval fixture 和 harness 任务状态文件。

分段 patch 是落盘策略，不要求一次性生成全部细节。对大内容首次写入前必须先形成足够稳定的全局框架，包括目标、影响范围、模块边界、接口关系、验证策略和整体复查计划；之后可以分模块递进式细化、实现和验证。禁止在没有全局框架时直接边写边扩散。

写入前必须判断内容规模：

- 单次 `apply_patch` 新增内容建议不超过 120 行。
- 单次 `apply_patch` 新增内容硬上限为 200 行；超过必须拆分。
- 当写入范围可能较大或无法判断规模时，必须先在计划中记录分段判断；实际新增超过 300 行时必须升级为分段写入。
- 目标文件超过 500 行时，默认禁止整文件重写，优先局部 patch。

这些限制对规划阶段同样生效。写 `execution-plan.md`、`environment.md`、`CHANGELOG.md` 或其他文档时，不能因为是文档就一次性写入大段内容。
...
禁止从代码块、JSON 对象、Markdown 表格或函数中间截断。若单个语义单元超过硬上限，应按内部结构继续拆分。

所有分段写入完成后，必须重新读取完整目标文件，检查命名、接口、章节、引用、占位符、重复内容和前后一致性；代码文件还必须结合项目测试、构建、lint 或静态检查验证整体效果。

如果 `apply_patch` 失败，必须先读取目标文件确认是否有部分写入，再判断失败原因：

- 上下文不匹配：重新读取相关片段后，只修正失败段。
- patch 过大：继续缩小段落，不重复写已成功段。
- 工具层错误：记录阻塞；如果最小 patch 仍失败，不用 shell 拼接文件绕过。
