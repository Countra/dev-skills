...
| lite/standard managed plan | `conditional` | 默认 same-context；风险/用户要求时委派 |
| low/medium-risk stage | `conditional` | 默认 same-context；风险/用户要求时委派 |
| standalone review | `conditional` | 默认 same-context；风险/用户要求时委派 |
| 用户或平台明确禁止委派 | `disabled` | 非独立回退；strict 调用方仍 blocked |

`conditional` 由风险策略确定，不由 Agent 自由选择。低/中风险且用户未要求独立审查时，coordinator 直接记录
`capability.status=policy-disabled`，不执行 Agent tool discovery，并完成完整 same-context 审查；风险 screen 命中高风险、
用户要求独立审查或调用方明确升级时，工具可用即必须委派。只有需要委派时才做一次直接检查或一次 tool discovery，并把
工具族、available/missing tools 固化到 preparation。`send_input` 只可用于一次纯 schema 修复，不能追加语义判断。
...
3. `timeout_class=standard` 默认等待 300 秒，显式旧预算仍可到 900 秒；`timeout_class=high-risk` 默认等待 900 秒，
   旧 artifact 的显式 1800 秒及按冻结目标规模的有界延长继续有效，但不得超过当前任务剩余预算。strict 固定为 high-risk；其它策略仅在冻结 brief 的
...
- conditional 风险策略禁用委派：直接生成 same-context fallback，记录 `REVIEW_DISPATCH_POLICY_DISABLED`，不得先做无意义的工具探测。
- conditional 确需委派但能力缺失：可生成 same-context fallback，记录 `REVIEW_HOST_TOOLS_UNAVAILABLE`。
...
正常 gating 路径优先使用组合入口，减少 finalize、assemble、validate 三次调用：

```powershell
python -u -X utf8 -B scripts/review_dispatch.py complete `
  --preparation <prepare.json> --outcome <outcome.json> `
  --dispatch-output <final-dispatch.json> --output <receipt.json> `
  --expected-dispatch-policy conditional `
  --workspace <workspace> --review-root <review-root>
```

`complete` 从 preparation 解析冻结 target/context/result 引用，先封存合法 dispatch，再组装和校验 receipt。语义结果或 receipt
校验失败时不生成 canonical passed receipt，但保留已合法写入的 dispatch 证据。旧 finalize、assemble、validate 入口继续用于
诊断、non-gating candidate 和历史流程。
