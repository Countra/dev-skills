...
Dispatch policy 只允许 `strict|conditional|disabled`。strict 能力缺失为 blocked；低/中风险 conditional 可由编排策略记录
`capability.status=policy-disabled` 并直接 same-context，风险升级或用户要求独立审查时工具可用必须 delegate；旧 conditional
external-agent receipt 继续有效。每个 dispatch 最多两个 attempt；attempt 2 必须绑定 retryable failed
attempt 1、完成必要关闭、在前序已创建 Agent 时使用不同 opaque Agent ID，并保持 target 不变；context 只有在前序明确请求
expansion 时才可做保留原条目的超集扩展。`timeout_class=standard` 默认 300 秒并兼容显式 900 秒，
`timeout_class=high-risk` 默认 900 秒，兼容旧 artifact 显式 1800 秒，并允许调用方按冻结目标规模显式延长但不得超过当前任务剩余预算；strict
