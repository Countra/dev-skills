...
description: 对复杂编码任务执行证据驱动、目标和上下文绑定且可复审的正式工程审查，仅提供 plan-review（规划方案审查）与 code-review（代码和变更审查）两个 profile。用于审查 managed task bundle、实施阶段 delta、最终集成变更或 standalone 本地代码目标，并生成 canonical JSON receipt；不得用于编写计划、修复代码、自动运行 Agent、操作远端平台或代替测试。
---
...
1. 明确 profile、scope、目标根目录、review root、实际 provenance，以及调用方提供的要求、规范、验证证据和风险 brief。
2. 生成 primary target；记录审查使用的上下文。不得把 review receipt 纳入其自身 target，也不得把实现者总结当作事实。
3. 先验证需求/批准意图，再审核心设计，最后覆盖全部分配范围；只有能命名具体风险时才扩展到 target 外上下文。
4. 执行 risk screen，只加载命中领域的 playbook；能力或证据不足时记录 verification gap，不伪造 N/A 或通过。
5. 逐个完成 profile lenses。finding、正向判断和 clean verdict 都必须有证据；偏好不得冒充阻断问题。
6. 建立新 receipt attempt 并运行公共 validator。目标或上下文变化后保留旧 attempt，完整复审并连接直属前序。
7. 需要人类视图时从已验证 JSON 渲染 Markdown；先报告 findings，再说明 verdict、限制和未覆盖面。
