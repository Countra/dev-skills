...
每条记录保留原 assertion evidence、usage、pairing 和 provenance。grade 同时保留 `run_state`、`run_status` 与可选 `run_error`；run fingerprint 缺失或 record 与 manifest fingerprint 冲突时拒绝评分，不能把不同实验条件混成一份 grade。
...
`se_report.py` 输出 JSON 与 Markdown，不输出单一 overall score；机器可读输出契约见 `schemas/report.schema.json`。重点字段：
...
`run_completed` 永远是 required gate；completed run 还必须通过 `run_status_consistent`，因此自报状态与 deterministic truth 冲突时仍可生成诊断报告，但不能得到全门禁通过。failed run 即使已有记录全部 PASS，也不能得到全门禁通过。没有对应 candidate mode 的质量 gate 标为 `not_applicable`，不会伪造通过率。required judge 只有在校准后给出 `authority=decision` 且选择 candidate 时通过；缺失或 advisory 证据均失败关闭。`all_required_passed` 只是逐项 gate 的合取结果，不是综合质量分。
