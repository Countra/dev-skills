...
```text
python -u -X utf8 -B scripts/se_doctor.py --live --probe-skill <skill-dir> --workspace <repo> --output <probe.json> --pretty
```

不传 `--probe-skill` 时 doctor 只检查版本；传入后才执行完整 capability probe。两种模式都不调用模型。完整 probe 检查：
...
6. 运行后同时重算 source tree 与 agent 实际可见的 skill tree，发现任一 drift 即失败关闭。
...
- runner provenance：adapter、CLI、model、sandbox、network、prompt hash、lab/skill hash、case、attempt、variant。

JSONL 出现未知 event type、成功退出却不能唯一证明 `turn.completed`、failed event、无 structured final、输出越界、timeout 或终止不确定时，不推断成功。先保留 artifact，再修复 capability 或 case。
