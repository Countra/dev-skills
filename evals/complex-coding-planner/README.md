...
- lite / standard / full 风险画像和对应 artifact 预算。
- `execution-plan.md` 与 `plan-contract.json` 的 ID、DAG、覆盖和 scope 一致性。
- Plan Quality Gate、Plan Self-Review、Plan Critique、Readiness 和 Approval。
...
- 缺失 contract、断链引用、环、可变计划章节、open decision 和缺少在线来源必须 fail closed。
...
runner 读取 `manifest.json`，分开报告 capability/regression，并输出 profile、计划行数和 artifact 数指标。沙箱限制临时目录时，可用 `--work-dir <path>` 指定隔离父目录。
