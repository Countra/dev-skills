...
- `ev_learn.py` 默认写基础 app/screen/element/workflow 摘要；只有显式 `--include-assets` 才写 action/workflow 资产。
...
| Stage 2 | report asset extraction smoke | pending | raw trace 清洗、riskFlags、参数化和候选资产 | 真实复跑 | pending | pending |
...
```json
{
  ...
  "next_automatic_action": "start Stage 1",
  "stop_condition": "none",
...
| Stage 1 | complete | 资产 schema 和存储层 | pass | py_compile、schema/store smoke、CLI smoke | commit Stage 1 |
| Stage 2 | pending | report 资产整理 | pending | pending | continue after Stage 1 commit |
...
| Stage 1 | pass | pass | pass | pending commit | yes | no | no | pass | pending | no | commit Stage 1 then continue Stage 2 |
...
- 已完成阶段（Completed stages）: planning, quality gate, self-review, readiness gate, approval, Stage 1 implementation/review/validation.
- 当前阶段（Current stage）: Stage 1 commit.
- 剩余阶段（Remaining stages）: Stage 1-5.
- 最新 commit（Latest commit）: pending Stage 1 commit.
- 下一步自动动作（Next automatic action）: commit Stage 1 and continue Stage 2.
...
- 复查后硬约束（Reviewed constraints）:
  ...
  - Stage 1 必须覆盖新 schema 初始化、旧库拒绝隐式迁移、显式重建、幂等初始化、去重和 CRUD smoke。
