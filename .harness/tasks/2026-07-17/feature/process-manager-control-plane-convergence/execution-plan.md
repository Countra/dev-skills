...
1. 将所有公共脚本及其 package export、`runtime_context/cli/client/config/errors/protocol` 边界迁移到 explicit context 和统一 JSON envelope/error taxonomy，删除 `default_config_path` CWD fallback 与 generic `manager_offline` 映射。
...
- Main / working branch：`harness/feature`，revision 3 规划时相对 `origin/harness/feature` ahead 2。
- Task type / branch action：feature planning；不创建或切换新 branch。
- Worktree known changes：本 task 未提交的 STG-02 source/tests 与 revision 3 planning/review artifacts；未观察到无关用户改动，激活 amendment 前重新核对 changed-path ownership。
- Commit authorization：revision 2 曾获阶段/最终提交授权；revision 3 正在重新请求，新的 attestation 写入前为 `not_authorized`。
...
- Current receipt：`artifacts/reviews/plan-review-attempt-4.json`。
...
- Requested scope：重新批准 revision 3 implementation，carry 已完成且定义不变的 STG-01，从当前未提交的 STG-02 实现继续并执行 STG-02..06。
...
| ART-08 | `artifacts/reviews/plan-review-attempt-4.json` | 当前 revision 的正式 plan-review receipt |

## Executor Handoff

Executor 获得 revision 3 approval 后必须：验证新 attestation 与 revision 2 archive，激活 amendment 并只 carry STG-01；从新的 STG-02 attempt 继续当前工作树；每阶段只改 allowed paths并执行 required validation 和 `complex-coding-reviewer` code-review；有 blocking/major finding 时修复并复审；只在用户授权下提交；最终 hosted evidence、final review 与 cleanup 全部闭合后才能完成任务。

- Planner checker：draft 后生成正式 receipt，再执行 approval mode。
- Open blocking decisions：none。
- Requested implementation authorization：yes，等待 revision 3 重新批准。
- Requested commit authorization：yes，等待 revision 3 重新批准。
