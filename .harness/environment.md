...
- 用户会话：当前任务要求 `process-manager` 对外使用统一跨平台方法并在内部自动适配 Windows Job Object、Linux delegated cgroup/process-group fallback 与 macOS process-group/kqueue；同时升级 `gitlab-pat-ops`，并直接适配 electron/planner/executor，不兼容旧版。
...
- 历史用户会话：最初的 `process-manager` 方案采用 Windows 与平台专属 launcher；当前实现已直接切换为跨平台 `direct`/`script` schema，该历史约束不再是 current contract。
...
- 当前仓库的 GitLab/process-manager/planner/executor/electron 能力以 Markdown、JSON/YAML 和 Python 3.11+ 标准库为主；process-manager 公共运行不依赖 PowerShell。
...
- JSON 检查：解析 pointer-only `.harness/active-task.json` 和 task `plan-contract.json`
- Planner 检查：`python -X utf8 -B skills/complex-coding-planner/scripts/harness_plan_check.py --task-dir <task-dir> --mode draft|approval --format text|json`
- Executor 检查：`python -X utf8 -B skills/complex-coding-executor/scripts/harness_exec_check.py --workspace . --task-dir <task-dir> --mode preflight|status|transition|reconcile|final`
...
- 关键规则检索：检索 `Research Gate`、`Research Drift Gate`、`Plan Amendment Gate`、`plan-contract`、`run-state`、`ledger`、`attestation`、`HARNESS_DISABLED` 和 `Topic Handoff`
...
- 浏览器/MCP: 本任务不需要。
- 外部服务: 普通本地开发不需要；三平台 release gate 使用无 secret 的 GitHub Actions hosted runners。
...
- `.harness/tasks/**/tmp/`、`.harness/tasks/**/logs/`、`.harness/process-manager/logs/`、`.harness/process-manager/runs/`、`.harness/process-manager/tmp/`、`.harness/process-manager/processes.json*`、`.harness/process-manager/token`、`.harness/process-manager/manager.json`、`.harness/process-manager/manager.lock` 和 launchd plist 默认忽略。
- `.harness/tasks/**/ledger*.jsonl`、`.harness/tasks/**/run-state*.json`、`.harness/tasks/**/attestation*.json`、`.harness/tasks/**/status*.json`、`.harness/sessions/` 默认作为运行时状态处理，除非任务计划明确要求提交。
...
- GitLab PAT Ops 的认证必填变量只使用 `SKILL_GITLAB_BASE_URL` 和 `SKILL_GITLAB_PAT`；可选安全配置同样限定为 `SKILL_GITLAB_*` 前缀，不读取通用或旧 token alias。缺少变量时 `gl_doctor.py --offline-check` 必须提示用户设置，不得泄露 token。
...
- 当前任务没有遗留长期后台服务；native smoke 是有 timeout/finally 的有限验证，并已通过统一 manager shutdown 与 owner-empty 证据收口。
- 后续必须启动长期服务时，按 Process Manager Gate 使用统一 `pm_manager.py`/`pm_*` CLI，记录 manager identity、processKey、ready、bounded logs 与 cleanup evidence；不判断 OS/backend，不手写后台 shell。
- `execution-plan.md` 解释不可变批准意图，`plan-contract.json` 提供机器约束；active pointer 不保存状态，运行事实只由 ledger replay 和 run-state snapshot 表达。
...
- 无长期 workspace 环境 blocker；任务级外部 runner/授权状态只记录在对应 task bundle。
